...
const transactionActionSchema: Schema = {
  ...
  properties: {
    actionType: {
      type: Type.STRING,
      enum: ["ADD_INCOME", "ADD_EXPENSE", "UPDATE_STOCK", "ADD_DEBT", "CHECK_STOCK", "CHECK_DEBTS", "MARK_DEBT_PAID", "CHECK_SUMMARY", "CHECK_TOP_PRODUCTS", "CHECK_TOP_CUSTOMERS", "CHECK_PAYMENT_METHODS", "UNKNOWN"],
...
8. If the user asks how much they made, spent, or profited over a period (e.g. "how much did I make today", "what's my profit this month", "how much did I spend this week", "how's business been"), output CHECK_SUMMARY. If they mentioned a relative window, set period to TODAY, THIS_WEEK, THIS_MONTH, or ALL_TIME (default ALL_TIME if neither period nor date is mentioned). If instead they asked about one specific past day (e.g. "yesterday", "how much did I make on the 20th", "last Monday"), set date (resolved to an absolute ISO date using today's real date above) and leave period blank. You do NOT have the real numbers yourself — do not guess a total in replyToUser, just acknowledge the question naturally.
9. If the user asks which product sells best/worst, has the highest/lowest revenue, is most/least popular, did well/poorly, or similar (e.g. "what product gave me the highest sales", "what's my best seller", "what did poorly today", "worst performing item"), output CHECK_TOP_PRODUCTS — NOT CHECK_SUMMARY, this is a different question. Set rank to BEST or WORST based on which direction they asked about (default BEST). Fill in period or date the same way as rule 8 if a time window was mentioned, otherwise leave both out. You do NOT know which product actually sold best/worst — do not guess a name in replyToUser, just acknowledge the question naturally.
10. If the user asks which customer spent the most/least, paid the highest/lowest, or similar (e.g. "which customer paid the highest", "who's my biggest spender"), output CHECK_TOP_CUSTOMERS. Set rank to BEST or WORST the same way as rule 9. Fill in period or date if a time window was mentioned. You do NOT know the real numbers — do not guess a name in replyToUser.
11. If the user asks about payment methods specifically — how many/much was paid via a method, or a breakdown across methods (e.g. "how many people paid with cash today", "how much came in via bank transfer this week", "break down my sales by payment method"), output CHECK_PAYMENT_METHODS — NOT CHECK_SUMMARY, this only concerns the payment-method split, not overall totals. Fill in paymentMethod only if the user asked about one specific method (Cash, Bank Transfer, Card); leave it blank for a full breakdown across all methods. Fill in period or date the same way as rule 8. You do NOT have the real numbers — do not guess in replyToUser.
12. Never force a question into an actionType that doesn't actually match it just because it's the closest option — if none of the actions above genuinely fit what the user asked, use UNKNOWN instead of guessing.
13. Only output UNKNOWN if, after considering the whole conversation, there is still no transaction or question of any kind in progress. If a transaction is in progress but a field (amount, product, customer, quantity) is still missing, output the correct actionType anyway with the fields you do have, and ask for the missing one in replyToUser — do NOT output UNKNOWN just because one field is missing.`;
}
